...
if __name__ == '__main__':
	...
	for rho in [-1, -0.5, 0, 0.5, 1]:	
		...
		K = 2
		clusters1 = mmdk_means(M, K, segments, ['Asset 1'])
		clusters2 = mmdk_means(M, K, segments, ['Asset 2'])
		
		coloring1 = []
		coloring2 = []

		for i in range(M):
		    W1 = [maximum_mean_discrepancy(segments[i][['Asset 1']], segments[clusters1[j]][['Asset 1']]) for j in range(K)]
		    coloring1.append(np.argmin(W1))

		    W2 = [maximum_mean_discrepancy(segments[i][['Asset 2']], segments[clusters2[j]][['Asset 2']]) for j in range(K)]
		...
		results1.to_csv(f'mmdk_mjd_results/results1_rho={rho}.csv', index = False)
		...
		results2.to_csv(f'mmdk_mjd_results/results2_rho={rho}.csv', index = False)
