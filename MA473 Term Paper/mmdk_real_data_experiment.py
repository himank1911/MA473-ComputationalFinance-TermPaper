import numpy as np
import pandas as pd
from mmdkmeans import maximum_mean_discrepancy, barycentre, mmdk_means
...
if __name__ == '__main__':	
	...
	clusters1 = mmdk_means(M, K, segments, [stocks[0]])
	clusters2 = mmdk_means(M, K, segments, [stocks[1]])
	
	coloring1 = []
	coloring2 = []

	for i in range(M):
	    W1 = [maximum_mean_discrepancy(segments[i][[stocks[0]]], segments[clusters1[j]][[stocks[1]]]) for j in range(K)]
	    coloring1.append(np.argmin(W1))

	    W2 = [maximum_mean_discrepancy(segments[i][[stocks[0]]], segments[clusters2[j]][[stocks[1]]]) for j in range(K)]
	...
	results1.to_csv('mmdk_real_results/results1.csv', index = False)
	...
	results2.to_csv('mmdk_real_results/results2.csv', index = False)
	...
	corr_clusters = mmdk_means(M, K, transformed_segments, stocks)
	...
	for i in range(M):
		...
		W = [maximum_mean_discrepancy(transformed_segments[i], transformed_segments[corr_clusters[j]]) for j in range(K)]
	...
	results_corr.to_csv('mmdk_real_results/results_corr.csv')
